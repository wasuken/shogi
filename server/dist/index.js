...
export function getAllLegalMoves(shogi) {
    const pseudoLegal = [];
    for (let x = 1; x <= 9; x++) {
        for (let y = 1; y <= 9; y++) {
...
export function parseCsaMoveToIMove(csaMove) {
    ...
    switch (pieceStr) {
        ...
        default:
            ...
            if (!kind)
                throw new Error(`Unknown piece kind in CSA move: ${pieceStr}`);
            break;
    }
...
app.get('/api/battle-results/:id/csa', async (c) => {
    ...
    try {
        ...
        return c.text(rows[0].record_text);
    }
    catch (error) {
